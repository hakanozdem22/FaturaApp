...
export default function ApprovedInvoicesScreen() {
    ...
    const fetchApprovedInvoices = async () => {
        ...
        try {
            ...
            // Muhasebe sadece faturaları görsün, Satın Alma sadece irsaliyeleri görsün
            if (profile?.role === 'muhasebe') {
                query = query.neq('document_type', 'İrsaliye');
            } else if (profile?.role === 'satinalma') {
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 p-8">
            <header className="flex flex-col gap-1">
                <h2 className="text-3xl font-bold text-slate-900 dark:text-white">
                    {profile?.role === 'muhasebe' ? 'Alım Onaylı Faturalar' : profile?.role === 'satinalma' ? 'Onaylı İrsaliyeler' : 'Onaylanan Faturalar'}
                </h2>
                <p className="text-slate-500 dark:text-slate-400">Sistemde onaylanmış olan tüm {profile?.role === 'satinalma' ? 'irsaliyeleri' : 'faturaları'} buradan görüntüleyebilir ve arayabilirsiniz.</p>
            </header>
